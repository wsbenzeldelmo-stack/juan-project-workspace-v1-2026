...
function formatCurrency(amount) {
    ...
    const app = (function() {
      ...
      let state = {
        ...
        catalogManagerType: "All Types",
        catalogManagerStatus: "All Status",
...
function openFeeInfo(type='all'){
  ...
  openModal('feeInfoModal');
      ...
      function catalogTableSnapshot(tableSelector){
        ...
        const clone=table.cloneNode(true);clone.removeAttribute('id');clone.querySelectorAll('[id]').forEach(el=>el.removeAttribute('id'));clone.querySelectorAll('button').forEach(btn=>btn.remove());clone.querySelectorAll('.popover-panel').forEach(x=>x.remove());
        return `<div class="catalog-full-table-scroll">${clone.outerHTML}</div>`;
      }
      function openCatalogFullView(){
        renderPricelist();const selected=state.catalogManagerCategory||'ALL',title=document.getElementById('catalogFullViewTitle'),body=document.getElementById('catalogFullViewBody');if(!body)return;
        const category=selected==='ALL'?'All Categories':displayCategory(selected);if(title)title.textContent=`Services & Packages · ${category}`;
        body.innerHTML=catalogTableSnapshot('#catalogServicesCard table');openModal('catalogFullViewModal');
      }
      function openCatalogSectionFullView(){openCatalogFullView();}
      function catalogDisplayName(name){return titleCaseProductName(normalizeBroadcastPackageName(name));}
      function catalogPrice(record,type){return type==='PACKAGE'?Math.max(0,Number(record.sellingPrice||0)):Math.max(0,Number(record.price||0));}
      function setCatalogTypeFilter(value){state.catalogManagerType=['Items','Packages'].includes(value)?value:'All Types';renderPricelist();}
      function setCatalogStatusFilter(value){state.catalogManagerStatus=['Active','Inactive'].includes(value)?value:'All Status';renderPricelist();}
      function setCatalogViewMode(mode='list'){state.catalogViewMode=mode==='grid'?'grid':'list';const card=document.getElementById('catalogServicesCard');card?.classList.toggle('shop-grid-mode',state.catalogViewMode==='grid');document.getElementById('shopListViewBtn')?.classList.toggle('active',state.catalogViewMode==='list');document.getElementById('shopGridViewBtn')?.classList.toggle('active',state.catalogViewMode==='grid');}
      function focusShopCategories(){const panel=document.getElementById('shopCategoriesPanel');panel?.scrollIntoView({behavior:'smooth',block:'center'});panel?.classList.add('shop-focus-pulse');setTimeout(()=>panel?.classList.remove('shop-focus-pulse'),850);}
      function handleShopCatalogImport(event){
        const input=event?.target,file=input?.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{try{const payload=JSON.parse(String(reader.result||'{}')),cats=Array.isArray(payload.categories)?payload.categories:[],items=Array.isArray(payload.items)?payload.items:[],packages=Array.isArray(payload.packages)?payload.packages:[];if(!cats.length&&!items.length&&!packages.length)throw new Error('No catalog arrays found.');ensureCatalogCategories();const existingItems=new Set(state.soloServices.map(x=>String(x.product_code||''))),existingPackages=new Set(state.packagesList.map(x=>String(x.product_code||'')));let addedItems=0,addedPackages=0;cats.map(x=>String(x||'').trim()).filter(Boolean).forEach(c=>{if(!state.catalogCategories.some(x=>x.toLowerCase()===c.toLowerCase()))state.catalogCategories.push(c)});items.forEach(x=>{if(!x||!x.product_code||existingItems.has(String(x.product_code)))return;state.soloServices.push({...x,active:x.active!==false,type:'SOLO'});existingItems.add(String(x.product_code));addedItems++;});packages.forEach(x=>{if(!x||!x.product_code||existingPackages.has(String(x.product_code)))return;state.packagesList.push({...x,active:x.active!==false,type:'PACKAGE',includedServiceNames:Array.isArray(x.includedServiceNames)?x.includedServiceNames:[]});existingPackages.add(String(x.product_code));addedPackages++;});ensureCatalogCategories();persistCatalogState();renderPricelist();renderServiceCatalog();showToast(`Catalog imported · ${addedItems} item${addedItems===1?'':'s'} · ${addedPackages} package${addedPackages===1?'':'s'}.`);}catch(err){console.warn('Catalog import failed:',err);showToast('Could not import catalog. Use a JUAN catalog JSON file.');}finally{if(input)input.value='';}};reader.onerror=()=>{showToast('Could not read the catalog file.');if(input)input.value='';};reader.readAsText(file);
      }
      function duplicateCatalogItem(code){const item=state.soloServices.find(x=>x.product_code===code);if(!item)return;const nums=state.soloServices.map(s=>Number(String(s.product_code||'').match(/(\d+)$/)?.[1]||0)),next=Math.max(0,...nums)+1;state.soloServices.push({...item,product_code:`SRV-${String(next).padStart(3,'0')}`,name:`${item.name} COPY`,active:false});persistCatalogState();renderPricelist();renderServiceCatalog();showToast('Item duplicated as inactive.');}
      function duplicateCatalogPackage(code){const pkg=state.packagesList.find(x=>x.product_code===code);if(!pkg)return;const nums=state.packagesList.map(p=>Number(String(p.product_code||'').match(/(\d+)$/)?.[1]||0)),next=Math.max(0,...nums)+1;state.packagesList.push({...pkg,product_code:`PKG-${String(next).padStart(3,'0')}`,name:`${pkg.name} COPY`,includedServiceNames:[...(pkg.includedServiceNames||[])],active:false});persistCatalogState();renderPricelist();renderServiceCatalog();showToast('Package duplicated as inactive.');}
      function toggleCatalogItemActive(code){const item=state.soloServices.find(x=>x.product_code===code);if(!item)return;item.active=item.active===false;persistCatalogState();renderPricelist();renderServiceCatalog();showToast(item.active?'Item activated.':'Item deactivated.');}
      function toggleCatalogPackageActive(code){const pkg=state.packagesList.find(x=>x.product_code===code);if(!pkg)return;pkg.active=pkg.active===false;persistCatalogState();renderPricelist();renderServiceCatalog();showToast(pkg.active?'Package activated.':'Package deactivated.');}
      function startCatalogRowDrag(event,kind,code){state.draggedCatalogRow={kind:String(kind||''),code:String(code||'')};event.dataTransfer.effectAllowed='move';event.currentTarget.classList.add('is-dragging');}
      function allowCatalogRowDrop(event){if(state.draggedCatalogRow)event.preventDefault();}
      function dropCatalogRow(event,kind,targetCode){event.preventDefault();document.querySelectorAll('.shop-catalog-row.is-dragging').forEach(x=>x.classList.remove('is-dragging'));const drag=state.draggedCatalogRow;state.draggedCatalogRow=null;if(!drag||drag.kind!==kind||drag.code===targetCode)return;const list=kind==='PACKAGE'?state.packagesList:state.soloServices,from=list.findIndex(x=>String(x.product_code||'')===drag.code),to=list.findIndex(x=>String(x.product_code||'')===String(targetCode||''));if(from<0||to<0)return;const [moved]=list.splice(from,1);list.splice(to,0,moved);persistCatalogState();state.listSorts.catalog='manual';renderPricelist();showToast(`${kind==='PACKAGE'?'Package':'Item'} sequence updated.`);}
      function renderPricelist(){
        ensureCatalogCategories();ensureAdditionalFees();
        const categoryList=document.getElementById('catalogCategoryList'),body=document.getElementById('catalogServicesBody');if(!categoryList||!body)return;
        const q=(document.getElementById('catalogManagerSearch')?.value||'').trim().toLowerCase(),sort=state.listSorts?.catalog||'manual',selected=state.catalogManagerCategory||'ALL',typeFilter=state.catalogManagerType||'All Types',statusFilter=state.catalogManagerStatus||'All Status';
        const typeEl=document.getElementById('catalogTypeFilter'),statusEl=document.getElementById('catalogStatusFilter'),sortEl=document.getElementById('catalogSort');if(typeEl)typeEl.value=typeFilter;if(statusEl)statusEl.value=statusFilter;if(sortEl)sortEl.value=sort;
        const categories=['ALL',...state.catalogCategories];
        categoryList.innerHTML=categories.map((category,index)=>{const label=category==='ALL'?'All':displayCategory(category),itemCount=category==='ALL'?state.soloServices.length:state.soloServices.filter(x=>String(x.category||'')===category).length,packageCount=category==='ALL'?state.packagesList.length:state.packagesList.filter(x=>String(x.category||'')===category).length,enc=encodeURIComponent(category),draggable=category!=='ALL'&&sort==='manual';return `<div class="catalog-category-item ${selected===category?'active':''}" ${draggable?'draggable="true"':''} data-service-index="${index-1}" ondragstart="app.startCatalogServiceDrag(event,${index-1})" ondragover="app.allowCatalogServiceDrop(event)" ondrop="app.dropCatalogService(event,${index-1})"><button class="catalog-category-select" type="button" onclick="app.selectCatalogCategory(decodeURIComponent('${enc}'))"><span class="service-drag-handle" aria-hidden="true">${category==='ALL'?'':'⋮⋮'}</span><span class="service-nav-copy"><strong>${escapeHtml(label)}</strong><small>${itemCount} item${itemCount===1?'':'s'} · ${packageCount} package${packageCount===1?'':'s'}</small></span></button>${category!=='ALL'?`<div class="popover-wrap" id="shopCategoryMenu_${index}"><button class="catalog-category-more vertical-more" type="button" aria-label="Actions for ${escapeHtml(label)}" onclick="event.stopPropagation();app.togglePopover('shopCategoryMenu_${index}',event)">⋮</button><div class="popover-panel client-row-menu"><button class="popover-action" onclick="app.openCatalogCategoryModal(decodeURIComponent('${enc}'))">Rename / Edit</button><button class="popover-action text-danger" onclick="app.openCatalogCategoryModal(decodeURIComponent('${enc}'))">Delete…</button></div></div>`:'<span></span>'}</div>`}).join('');
        const matchesStatus=x=>statusFilter==='All Status'||(statusFilter==='Active'?(x.active!==false):(x.active===false));
        let records=[];
        if(typeFilter!=='Packages')state.soloServices.forEach(item=>{if((selected==='ALL'||String(item.category||'')===selected)&&matchesStatus(item)&&(`${item.name||''} ${item.category||''} ${item.description||''}`).toLowerCase().includes(q))records.push({kind:'ITEM',record:item});});
        if(typeFilter!=='Items')state.packagesList.forEach(pkg=>{if((selected==='ALL'||String(pkg.category||'')===selected)&&matchesStatus(pkg)&&(`${pkg.name||''} ${pkg.category||''} ${pkg.description||''} ${(pkg.includedServiceNames||[]).join(' ')}`).toLowerCase().includes(q))records.push({kind:'PACKAGE',record:pkg});});
        if(sort==='az')records.sort((a,b)=>String(a.record.name||'').localeCompare(String(b.record.name||'')));else if(sort==='za')records.sort((a,b)=>String(b.record.name||'').localeCompare(String(a.record.name||'')));else if(sort==='price-asc')records.sort((a,b)=>catalogPrice(a.record,a.kind)-catalogPrice(b.record,b.kind));else if(sort==='price-desc')records.sort((a,b)=>catalogPrice(b.record,b.kind)-catalogPrice(a.record,a.kind));else records.sort((a,b)=>{const ai=a.kind==='ITEM'?state.soloServices.indexOf(a.record):state.packagesList.indexOf(a.record),bi=b.kind==='ITEM'?state.soloServices.indexOf(b.record):state.packagesList.indexOf(b.record);return String(a.record.category||'').localeCompare(String(b.record.category||''))||ai-bi;});
        body.innerHTML=records.map(({kind,record},i)=>{const isPackage=kind==='PACKAGE',inc=isPackage&&Array.isArray(record.includedServiceNames)?record.includedServiceNames:[],active=record.active!==false,code=String(record.product_code||''),menu=`shopCatalogRow_${kind}_${i}`,name=catalogDisplayName(record.name),category=displayCategory(record.category||''),price=catalogPrice(record,kind),original=isPackage?Math.max(0,Number(record.originalPrice||0)):0;return `<tr class="shop-catalog-row ${isPackage?'package-row':''} ${active?'':'is-inactive'}" ${sort==='manual'?`draggable="true" ondragstart="app.startCatalogRowDrag(event,'${kind}','${code}')" ondragover="app.allowCatalogRowDrop(event)" ondrop="app.dropCatalogRow(event,'${kind}','${code}')"`:''}><td data-label="Item / Package"><strong>${escapeHtml(name)}</strong><small class="row-subtype">${escapeHtml(category)}${!isPackage&&record.description?` · ${escapeHtml(record.description)}`:''}</small></td><td data-label="Type"><span class="shop-type-badge ${isPackage?'package':'item'}">${isPackage?'Package':'Item'}</span></td><td data-label="Inclusions">${isPackage?`<details class="catalog-inclusions-accordion"><summary>${inc.length} item${inc.length===1?'':'s'}</summary><div>${inc.map(x=>`<span>${escapeHtml(titleCaseProductName(x))}</span>`).join('')}</div></details>`:'<span class="shop-dash">—</span>'}</td><td class="shop-price-cell" data-label="Price"><strong>${formatCurrency(price)}</strong>${isPackage&&original>0&&original!==price?`<small class="row-price-note">Original ${formatCurrency(original)}</small>`:''}</td><td data-label="Status"><span class="shop-status-pill ${active?'active':'inactive'}">${active?'Active':'Inactive'}</span></td><td class="actions-cell" data-label="Actions"><div class="popover-wrap" id="${menu}"><button class="icon-more-button vertical-more" aria-label="Actions for ${escapeHtml(name)}" onclick="app.togglePopover('${menu}',event)">⋮</button><div class="popover-panel client-row-menu"><button class="popover-action" onclick="${isPackage?`app.openEditPackageModal('${code}')`:`app.openEditServiceModal('${code}')`}">Edit</button><button class="popover-action" onclick="${isPackage?`app.duplicateCatalogPackage('${code}')`:`app.duplicateCatalogItem('${code}')`}">Duplicate</button><button class="popover-action" onclick="${isPackage?`app.openEditPackageModal('${code}')`:`app.openEditServiceModal('${code}')`}">Move / Change Category</button><button class="popover-action" onclick="${isPackage?`app.toggleCatalogPackageActive('${code}')`:`app.toggleCatalogItemActive('${code}')`}">${active?'Deactivate':'Activate'}</button><button class="popover-action text-danger" onclick="${isPackage?`app.quickDeleteCatalogPackage('${code}')`:`app.quickDeleteCatalogItem('${code}')`}">Delete</button></div></div></td></tr>`}).join('')||`<tr><td colspan="6"><div class="empty-compact-state"><strong>No matching services or packages</strong><span>Try another search, category, type, or status filter.</span><button class="btn btn-secondary btn-sm" type="button" onclick="app.resetShopFilters()">Reset filters</button></div></td></tr>`;
        setCatalogViewMode(state.catalogViewMode||'list');
      }
      function resetShopFilters(){state.catalogManagerCategory='ALL';state.catalogManagerType='All Types';state.catalogManagerStatus='All Status';state.listSorts.catalog='manual';const search=document.getElementById('catalogManagerSearch');if(search)search.value='';renderPricelist();}
      ...
      function setCatalogManagerFilter(filter){state.catalogManagerType=filter==='Packages'?'Packages':filter==='Items'?'Items':'All Types';renderPricelist()}
      function selectCatalogCategory(category){state.catalogManagerCategory=category||'ALL';renderPricelist()}
      ...
      function defaultAdditionalFees(){
        return [
          {code:'RUSH',name:'Rush Fee',amount:500,rule:'per_4_days_early',description:'Applied when the requested completion date is earlier than the standard project turnaround.',active:true,locked:true},
          {code:'REVISION',name:'Revision Fee',amount:500,rule:'per_revision',description:'Applied for revision requests beyond the included revision allowance.',active:true,locked:true},
          {code:'SYSTEM_MAINTENANCE',name:'System Maintenance Fee',amount:21,rule:'package_once',description:'A fixed system and project maintenance charge applied to eligible orders.',active:true,locked:true}
        ];
      }
      ...
      function renderAdditionalFeesManager(){
        const body=document.getElementById('catalogAdditionalFeesBody');if(!body)return;ensureAdditionalFees();const ruleLabel={per_4_days_early:'Every 4 days earlier',per_revision:'Per additional revision',package_once:'Once per applicable order',manual:'Manual'};
        body.innerHTML=(state.additionalFees||[]).map((fee,i)=>{const menu=`shopFee_${i}`,active=fee.active!==false;return `<tr><td><strong>${escapeHtml(fee.name||'Fee')}</strong>${fee.locked?'<small class="row-subtype">Protected core fee</small>':'<small class="row-subtype">Custom fee</small>'}</td><td class="shop-price-cell"><strong>${formatCurrency(fee.amount||0)}</strong></td><td>${escapeHtml(ruleLabel[fee.rule]||'Manual')}</td><td><span class="shop-status-pill ${active?'active':'inactive'}">${active?'Active':'Inactive'}</span></td><td><button class="inline-info-button" type="button" aria-label="View information for ${escapeHtml(fee.name||'fee')}" onclick="app.openFeeInfo('${escapeHtml(fee.code)}')">i</button></td><td class="actions-cell"><div class="popover-wrap" id="${menu}"><button class="icon-more-button vertical-more" aria-label="Actions for ${escapeHtml(fee.name||'fee')}" onclick="app.togglePopover('${menu}',event)">⋮</button><div class="popover-panel client-row-menu"><button class="popover-action" onclick="app.openAdditionalFeeModal('${escapeHtml(fee.code)}')">Edit Fee</button>${fee.locked?'':`<button class="popover-action text-danger" onclick="app.deleteAdditionalFee('${escapeHtml(fee.code)}')">Delete Fee</button>`}</div></div></td></tr>`}).join('')||'<tr><td colspan="6" class="text-center text-muted py-4">No fees configured.</td></tr>';
      }
      function openAdditionalFeesManager(){renderAdditionalFeesManager();openModal('additionalFeesManagerModal');}
      ...
      return {
        ...
        resetShopFilters,
        focusShopCategories,
        handleShopCatalogImport,
        duplicateCatalogItem,
        duplicateCatalogPackage,
        toggleCatalogItemActive,
        toggleCatalogPackageActive,
        openAdditionalFeesManager,
